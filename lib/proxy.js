...
function HttpProxy (options) {
  ...
  if (this.cache.enabled){
    ...
    //TTL check interval
    setInterval(this.cleanUp, this.cache.ttl / 2);
    this.cleanUp();
  };
...
HttpProxy.prototype.handleRequest = function (req, res) {
  ...
  if (this.auth && !req.headers["proxy-authorization"] ||
      (new Buffer(req.headers["proxy-authorization"].split(" ")[1], "base64")).toString() != this.auth) {
    res.statusCode = 401;
    res.end();
  };
